...
const BASE_URL = "https://financeview-backend.onrender.com";
...
export async function getStocksInfo(
  ...
  const response = await fetch(`${BASE_URL}/stock/${ticker}/${range}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function sendReview(
  ...
  const response = await fetch(`${BASE_URL}/user/feedback`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
export async function getUserImage(userId: string, token: string | null) {
  const response = await fetch(`${BASE_URL}/user/image/${userId}`, {
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
    },
